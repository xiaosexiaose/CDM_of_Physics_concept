...
#连接成绩
    ...
    final_data_of_construct_xiangdui <- read_csv("D:/ELETRICITY_DATA/final_data_of_construct_xiangdui.csv")
...
names(find_cluster_data_xiangdui)<-c("平均路径长度","最长路径","结构密度系数","聚类系数","节点连通度","边连通度")
# View(find_cluster_data_xiangdui)
...
#聚类参数选择方法3
theGap_xiangdui<-clusGap(find_cluster_best_xiangdui,FUNcluster = pam,K.max = 30)
Gapdata_xiangdui<-as.data.frame(theGap_xiangdui$Tab)
Gapdata_xiangdui

ggplot(Gapdata_xiangdui,aes(x=1:nrow(Gapdata_xiangdui)))+geom_line(aes(y=gap))+geom_point(aes(y=gap))+geom_errorbar(aes(ymin=gap-SE.sim,ymax=gap+SE.sim))+labs(x="聚类数目",y="Gap")+theme_bw()+theme(panel.border = element_blank())+scale_x_continuous(breaks = seq(1,30,1))
...
p2_xiangdui<-ggplot(data = find_cluster_data_xiangdui_final,aes(x=聚类系数,y=结构密度系数,color=cluster))+geom_point()+theme(legend.position = "right")+geom_point(data = centers_xiangdui,aes(x=聚类系数,y=结构密度系数,color=as.factor(c(1,2,3))),size=10,alpha=0.3,show.legend=FALSE)+scale_color_hue("概念结构分类")
...
p4_xiangdui<-ggplot(data = find_cluster_data_xiangdui_final,aes(x=结构密度系数,y=节点连通度,color=cluster))+geom_point()+theme(legend.position = "right")+geom_point(data = centers_xiangdui,aes(x=结构密度系数,y=节点连通度,color=as.factor(c(1,2,3))),size=10,alpha=0.3,show.legend=FALSE)+scale_color_hue("概念结构分类")

p5_xiangdui<-ggplot(data = find_cluster_data_xiangdui_final,aes(x=结构密度系数,y=平均路径长度,color=cluster))+geom_point()+theme(legend.position = "right")+geom_point(data = centers_xiangdui,aes(x=结构密度系数,y=平均路径长度,color=as.factor(c(1,2,3))),size=10,alpha=0.3,show.legend=FALSE)+scale_color_hue("概念结构分类")
...
#构建决策树
fit_tree<-rpart(cluster~平均路径长度+结构密度系数+聚类系数+节点连通度,data = find_cluster_data_xiangdui_final,method="class",control=rpart.control(minsplit = 1),parms=list(split='information'))
summary(fit_tree)
...
#筛选恰当分类的学生
```{r}
View(final_data_of_construct_anylisis)

costruct_of_1<-final_data_of_construct_anylisis[(final_data_of_construct_anylisis$liantong_nodes==1 & final_data_of_construct_anylisis$average_path_constr<0.92),]
View(costruct_of_1)
...
costruct_of_2<-final_data_of_construct_anylisis[(final_data_of_construct_anylisis$liantong_nodes==1 & final_data_of_construct_anylisis$average_path_constr>=0.92),]
View(costruct_of_2)
costruct_2<-costruct_of_2$X
for (costruc2 in costruct_2) {
  plot(my_figurelist[[costruc2]])
}

costruct_of_3<-final_data_of_construct_anylisis[(final_data_of_construct_anylisis$liantong_nodes==0),]
View(costruct_of_3)
